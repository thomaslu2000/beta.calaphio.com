...
const Comments = props => {
  ...
  const submitComment = async () => {
    if (comm !== '') {
      ...
      await axios
        .post(`${API_URL}/events/postComment/`, item, {
  ...
  const deleteComment = async id => {
    await axios
      .post(
        `${API_URL}/events/deleteComment/`,
        { commentId: id, API_SECRET },
        { headers: { 'content-type': 'application/x-www-form-urlencoded' } }
      )
      .then(response => {
  ...
  return (
    <div className={classes.root}>
                ...
                <ListItemText
                  ...
                  secondary={
                    <React.Fragment>
                      ...
                      {'   --'} {moment.utc(comment.timestamp).fromNow()}{' '}
                      {comment.comment_id && comment.user_id === global.userId && (
                        <IconButton
                          size="small"
                          style={{ color: 'Crimson' }}
      ...
      <TextField
        ...
        InputProps={{
          endAdornment: <Button onClick={submitComment}>Submit</Button>
        }}
      />
    </div>
  );
