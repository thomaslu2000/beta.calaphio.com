...
const columns = [
  ...
  valueFormatter: (params) =>
  params.value || 0},
  { field: 'category', headerName: 'Type', flex: 0.7, 
  valueGetter: (r) => {
    return r.row.service==="1" ? 'Service' : r.row.fellowship==="1" ? 'Fellowship' : 'Other' }}, 
  { field: 'attended', headerName: 'Attendence', flex: 0.7, 
  valueGetter: (r) => {
...
const useStyles = makeStyles(theme => ({
  ...
  centered : {
    marginLeft:'auto', marginRight:'auto', alignText:'center' 
  }
}));
...
const Positions = props => {
  ...
  if (years.reduce((sum, year) => {
    if (userData[year]) return sum + userData[year][0].length + userData[year][1].length
    return sum
  }, 0) === 0) {
    return <div></div>
  }
  ...
  <Paper  style={{ width:'100%'}}>
    <Grid container alignItems="center" justify="center" direction="column" style={{paddingTop: 10, paddingBottom: 10}}>
    <Grid item xs={12} style={{paddingTop: 10, paddingBottom: 10}}>
        <Typography variant='h2' align='center' >Excomm Positions</Typography> 
      </Grid>
      <Divider style={{paddingTop: 5, paddingBottom: 5}}/>
    {years.map(year => {
      return <React.Fragment key={year}>
      {[0, 1].map(sem => {
        if (!userData[year] || userData[year][sem].length === 0) return '';
        return <Grid item xs={12} key={year + sem} style={{paddingTop: 10, paddingBottom: 15}}>
        <Typography variant='h4' align='center'>{['Spring', 'Fall'][sem]} {year}</Typography> 
        {userData[year][sem].map(row => 
          <Typography variant='h6'  align='center'> {row['position_title']} -- {row['position_name']}</Typography> 
        )}
      </Grid>
      }
      )}
